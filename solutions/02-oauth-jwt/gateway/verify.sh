#!/usr/bin/env bash
# Solution 02 — proof of the OAuth 2.0 client-credentials flow.
#
...
case "$status" in
  ...
  403) fail "valid token → 403. Authentication succeeded but authorization did not — is api-product-enforcer on this route with no subscription behind it? See solution 01." ;;
