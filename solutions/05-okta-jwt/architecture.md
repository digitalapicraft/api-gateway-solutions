# Architecture — verifying Okta-issued tokens at the gateway

The gateway takes over exactly one responsibility here, and deliberately not the
one people expect: it becomes the **verifier** of tokens, never the issuer. Okta
keeps the private key, the user directory, the grant logic and the token
lifetime. The gateway keeps a cached copy of Okta's public keys and an opinion
about which issuer it trusts.
